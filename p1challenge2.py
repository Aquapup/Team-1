...
def update_contour():
    ...
    Kp=1
    area_to_turn=4000
    area_to_straight=700
    area_notStraight=1200
    areaToTurnBack=200

    if blue1red0==0:

        red_contour = rc_utils.get_largest_contour(rc_utils.find_contours(image, RED[0], RED[1]),200)
    ...
    #CROP!!!!!
        ...
        blue_contour = rc_utils.get_largest_contour(rc_utils.find_contours(image, BLUE[0], BLUE[1]),200)
        #rc_utils.draw_contour(image, red_contour)
        rc.display.show_color_image(image)
        #rc_utils.draw_contour(image, red_contour)
    # rc_utils.draw_circle(image, contour_center)    
...
#CROP!!!!!
        ...
        try:
            if(bcontour_area>area_to_turn) and (s_phase!=2):
                rc.drive.set_speed_angle(1,-1)
                s_phase=2
            elif(area_to_straight<bcontour_area<area_notStraight and (s_phase!=2)):
                straight_angle= Kp*(((float)(bcontour_center[1] )/ rc.camera.get_width()) * 2 - 1)                
                print("blue straight angle",straight_angle)
                rc.drive.set_speed_angle(1,straight_angle)
                s_phase=1
            elif(bcontour_area<areaToTurnBack) and (s_phase==2):
                rc.drive.set_speed_angle(1,1)
                blue1red0=0
                s_phase=0
            elif(s_phase==1):
